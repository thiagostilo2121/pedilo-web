...
function Section({ icon: Icon, title, count, children, color = "text-gray-900", bgColor = "bg-gray-50" }) {
        ...
        <div className="border border-gray-100 rounded-2xl overflow-hidden bg-white">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    {count > 0 && (
                        <span className="text-[10px] font-bold bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{count}</span>
                    )}
                </div>
...
export default function IntelligenceCard() {
    ...
    if (loading) {
        return (
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 flex flex-col items-center justify-center min-h-[300px] gap-4">
                <div className="relative">
                    <div className="w-16 h-16 bg-gradient-to-br from-violet-100 to-indigo-100 rounded-2xl flex items-center justify-center animate-pulse">
                ...
                <div className="text-center">
                    <p className="font-bold text-gray-900 text-sm">Analizando tu negocio...</p>
                    <p className="text-[11px] text-gray-400 mt-1">Procesando datos de pedidos, clientes y productos</p>
                </div>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                ...
                <button onClick={loadData} className="mt-2 text-xs font-bold text-gray-500 underline">Reintentar</button>
            </div>
        );
    }
            ...
            <div className="p-5 sm:p-6 border-b border-gray-100 bg-gradient-to-r from-violet-50 via-white to-indigo-50">
                <div className="flex items-start sm:items-center justify-between flex-col sm:flex-row gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="flex items-center gap-1.5 bg-violet-50 text-violet-700 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider border border-violet-200">
                            <Zap size={12} />
                            {totalActions} Accion{totalActions !== 1 ? "es" : ""}
                        </span>
                        <button onClick={loadData} className="p-2 rounded-xl bg-gray-50 hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-all" title="Actualizar">
                            <RefreshCcw size={14} />
                        </button>
                    </div>
                </div>
            ...
            <div className="p-4 sm:p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 p-4 rounded-2xl border border-emerald-200/50">
                        ...
                        <div className="font-black text-emerald-700 text-lg sm:text-xl tracking-tight">
                            {formatMoney(roi_simulation.ahorro_marketplace_30)}
                        </div>
                        <div className="text-[10px] text-emerald-500 font-medium mt-0.5">vs 30% comisión</div>
                    </div>
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 p-4 rounded-2xl border border-blue-200/50">
                        ...
                        <div className="font-black text-blue-700 text-lg sm:text-xl tracking-tight">
                            {formatMoney(roi_simulation.ahorro_anual_estimado)}
                        </div>
                        <div className="text-[10px] text-blue-500 font-medium mt-0.5">proyección neta</div>
                    </div>
                    <div className="bg-gradient-to-br from-violet-50 to-violet-100/50 p-4 rounded-2xl border border-violet-200/50">
                        <div className="text-[10px] font-bold text-violet-600 uppercase tracking-wider mb-1 flex items-center gap-1">
                            <Target size={10} /> Forecast 7d
                        </div>
                        <div className="font-black text-violet-700 text-lg sm:text-xl tracking-tight">
                            {demand_forecast.pedidos_estimados_7d || 0}
                            <span className="text-xs font-bold ml-1 text-violet-400">pedidos</span>
                        </div>
                        <div className="text-[10px] text-violet-500 font-medium mt-0.5 flex items-center gap-1">
                    ...
                    <div className="bg-gradient-to-br from-orange-50 to-orange-100/50 p-4 rounded-2xl border border-orange-200/50">
                        ...
                        <div className="font-black text-orange-700 text-lg sm:text-xl tracking-tight">
                        ...
                        {retention_actions.map((action, i) => (
                            <div key={i} className="p-4 rounded-xl bg-gray-50 border border-gray-100 hover:border-gray-200 transition-all">
                                <div className="flex items-start justify-between gap-2 mb-2">
                                    <h5 className="font-bold text-gray-900 text-sm leading-tight">{action.titulo}</h5>
                                    <PriorityBadge priority={action.prioridad} />
                                </div>
                                <p className="text-xs text-gray-600 leading-relaxed mb-3">{action.descripcion}</p>
                                ...
                                {action.clientes && (
                                    <div className="space-y-1.5 mb-3">
                                        {action.clientes.map((c, j) => (
                                            <div key={j} className="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-gray-100 text-xs">
                                                <div className="flex items-center gap-2 min-w-0">
                                                    <div className="w-7 h-7 bg-gray-900 text-white rounded-lg flex items-center justify-center font-bold text-[10px] shrink-0">
                                                        {c.nombre?.charAt(0).toUpperCase() || "?"}
                                                    </div>
                                                    <span className="font-bold text-gray-900 truncate">{c.nombre}</span>
                                                </div>
                                                <div className="flex items-center gap-2 shrink-0">
                                                    <span className="font-bold text-gray-500">{formatMoney(c.gastado)}</span>
                                                    {c.telefono && (
                                ...
                                {action.sugerencia_cupon && (
                                    <div className="flex items-center gap-2 bg-violet-50 px-3 py-2 rounded-lg border border-violet-100">
                                        ...
                                        <span className="text-[11px] text-violet-700 font-bold">Cupón sugerido: <code className="bg-violet-100 px-1.5 py-0.5 rounded text-violet-800">{action.sugerencia_cupon}</code></span>
                                    </div>
                                )}
                                ...
                                {action.horas_muertas && (
                                    <div className="flex flex-wrap gap-1.5 mt-2">
                                        ...
                                        {action.horas_muertas.map((h, j) => (
                                            <span key={j} className="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-bold">{h}</span>
                                        ))}
                                    </div>
                                )}
                            </div>
                    ...
                    <Section icon={Sparkles} title="Combos Sugeridos" count={combo_suggestions.length} color="text-violet-600" bgColor="bg-violet-50">
                        {combo_suggestions.map((combo, i) => (
                            <div key={i} className="p-4 rounded-xl bg-gray-50 border border-gray-100">
                                <div className="flex items-start justify-between gap-2 mb-2">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        {combo.productos.map((p, j) => (
                                            <span key={j}>
                                                <span className="font-bold text-gray-900 text-sm">{p}</span>
                                ...
                                <p className="text-xs text-gray-600 mb-2">{combo.accion}</p>
                                <div className="flex items-center gap-4 text-[11px] font-bold">
                                    <span className="text-gray-500">{combo.frecuencia}x comprados juntos</span>
                ...
                {promo_recommendations.length > 0 && (
                    <Section icon={Target} title="Promos Recomendadas" count={promo_recommendations.length} color="text-orange-600" bgColor="bg-orange-50">
                        {promo_recommendations.map((promo, i) => (
                            <div key={i} className="p-4 rounded-xl bg-gray-50 border border-gray-100">
                                <div className="flex items-start justify-between gap-2 mb-2">
                                    <h5 className="font-bold text-gray-900 text-sm">{promo.producto}</h5>
                                    <PriorityBadge priority={promo.prioridad} />
                                </div>
                                <p className="text-xs text-gray-600 mb-2">{promo.accion}</p>
                ...
                {dead_products.length > 0 && (
                    <Section icon={PackageX} title="Productos Sin Movimiento" count={dead_products.length} color="text-gray-600" bgColor="bg-gray-100">
                        <div className="space-y-2">
                            {dead_products.map((prod, i) => (
                                <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-100 text-xs">
                                    <div className="flex items-center gap-2 min-w-0">
                                        <PackageX size={14} className="text-gray-400 shrink-0" />
                                        <span className="font-bold text-gray-900 truncate">{prod.nombre}</span>
                                        {prod.categoria && (
                                            <span className="text-[10px] bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded-full shrink-0">{prod.categoria}</span>
                                        )}
                                    </div>
                                    <span className="font-bold text-gray-500 shrink-0">{formatMoney(prod.precio)}</span>
                                </div>
                            ))}
                        </div>
                ...
                <Section icon={DollarSign} title="Simulador de ROI" count={0} color="text-emerald-600" bgColor="bg-emerald-50">
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-500 font-medium">Ventas este periodo</span>
                            <span className="font-bold text-gray-900">{formatMoney(roi_simulation.ventas_periodo)}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-500 font-medium">Si usaras App (30%)</span>
                            <span className="font-bold text-red-500">-{formatMoney(roi_simulation.ahorro_marketplace_30)}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-500 font-medium">Costo Pedilo (mensual)</span>
                            <span className="font-bold text-gray-900">-{formatMoney(roi_simulation.costo_pedilo_mensual)}</span>
                        </div>
                        <div className="border-t border-gray-200 pt-3 flex justify-between text-sm">
                            <span className="font-bold text-gray-900">Ganancia Neta vs Apps</span>
                    ...
                    <p className="text-[11px] text-gray-500 font-medium italic">{roi_simulation.mensaje}</p>
                </Section>
                ...
                <Section icon={TrendingUp} title="Pronóstico de Demanda" count={0} color="text-blue-600" bgColor="bg-blue-50">
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 text-center">
                            <div className="text-[10px] text-gray-400 font-bold uppercase mb-1">Pedidos / Día</div>
                            <div className="text-xl font-black text-gray-900">{demand_forecast.promedio_diario_pedidos || 0}</div>
                        </div>
                        <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 text-center">
                            <div className="text-[10px] text-gray-400 font-bold uppercase mb-1">Ingreso / Día</div>
                            <div className="text-xl font-black text-gray-900">{formatMoney(demand_forecast.promedio_diario_ventas)}</div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] font-medium text-gray-500 mt-1">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${demand_forecast.confianza === 'alta' ? 'bg-green-50 text-green-600' : demand_forecast.confianza === 'media' ? 'bg-yellow-50 text-yellow-600' : 'bg-gray-100 text-gray-500'}`}>
